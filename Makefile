linode_install:
	...
	sudo cp resources/default.conf /etc/nginx/sites-available/fastapi_app
...
linode_run:
	...
	poetry run gunicorn --bind=unix:///tmp/uvicorn.sock -w 4 --forwarded-allow-ips='*' -k uvicorn.workers.UvicornWorker techzapi:app
